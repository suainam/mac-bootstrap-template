# knowledge-record / lifecycle-manager record — Design
...
└── scripts/
    ...
    └── record_knowledge.py  # compatibility shim -> knowledge-record owner
```
...
template/.venv/bin/python \
  ...
  --content "这是手动创建的测试记录。" \
  --tags "测试记录,知识沉淀" \
  --background "记录入口恢复后的 contract 示例。" \
  --references "template/docs/data-hub-record-knowledge.md" \
  --why-record "保留 record owner 与管理入口分离的设计。" \
  --agent codex \
  --date "$(date +%F)"
```
