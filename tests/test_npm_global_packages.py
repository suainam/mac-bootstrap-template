...
def test_install_script_tolerates_nonzero_npm_ls_with_valid_json(tmp_path: Path):
    ...
    npm_script.write_text(
        ...
        "set -euo pipefail\n"
        "if [ \"$1\" = \"-g\" ] && [ \"$2\" = \"ls\" ]; then\n"
        f"  python3 -c 'import json; from pathlib import Path; pkgs = [line.strip() for line in Path(\"{TEMPLATE}/agent/npm-global-packages.txt\").read_text().splitlines() if line.strip() and not line.startswith(\"#\")]; print(json.dumps({{\"dependencies\": {{p: {{}} for p in pkgs}}}}))'\n"
        "  exit 1\n"
        "fi\n"
