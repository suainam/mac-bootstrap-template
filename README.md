...
| `make patch-chrome-gemini [USER=<username>] [KILL=1]` | Apply patch to enable Gemini features in Chrome Local State (portable in-place patch); `KILL=1` terminates Chrome first and fails if Chrome stays running after a bounded wait (safe to rerun) |
