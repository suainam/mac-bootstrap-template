...
load_x_mcp_private_env() {
  ...
  config_path="$private_dir/agent/x-mcp.jsonc"
...
client_credential = oauth.get("client_secret") or ""
callback_url = oauth.get("callback_url") or ""

if enabled is True:
    print("X_MCP_ENABLE=1")
elif enabled is False:
    print("X_MCP_ENABLE=0")

if client_id and client_id != "REPLACE_ME":
    print(f"X_MCP_CLIENT_ID={client_id}")
if client_credential and client_credential != "REPLACE_ME":
    print(f"X_MCP_CLIENT_SECRET={client_credential}")
if callback_url:
    print(f"X_MCP_CALLBACK_URL={callback_url}")
PY
)" || return 1
