...
- `mac-bootstrap-parent` requires staged `template` gitlink OIDs to be fetchable, then runs the fixed `repo-check` target before push. `machine-check` additionally requires `agent.runtime.managementCheckout=true` in a primary non-submodule checkout; ordinary clones and linked worktrees remain repo-only. The trusted bundle owns the target selection and doctor reports the effective scope.
