...
- Second host tracer: `template/scripts/agent_opencode_edit_adapter.py` maps OpenCode `tool.execute.after` for `write`, `edit`, and single-file `apply_patch` to the same `after.edit` event. It reuses the existing profile, runtime, gate, diagnostic fingerprint, and Git dispatcher without changing their contracts.
- The host adapters are thin drivers, not the abstraction boundary; no generic adapter framework or plugin loader is introduced.
...
- `mac-bootstrap-parent` requires staged `template` gitlink OIDs to be fetchable, then runs the fixed `repo-check` target before push. `machine-check` additionally requires `agent.runtime.managementCheckout=true` in a primary non-submodule checkout; ordinary clones and linked worktrees remain repo-only. The trusted bundle owns the target selection, passes the install-selected external Python to Make without resolving away a venv entrypoint, and doctor reports the effective scope.
