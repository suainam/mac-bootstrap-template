...
template/.venv/bin/python template/agent/data-hub/daily_summary.py "$TARGET_DATE"

if [[ "$APPLY_REVIEWED" == "1" ]]; then
  template/.venv/bin/python template/agent/data-hub/materialize_candidates.py "$TARGET_DATE"
fi
