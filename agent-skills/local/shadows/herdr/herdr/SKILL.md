...
## Local authorization and data-handling policy

This reviewed local shadow adds the following policy to the upstream Herdr workflow:

- Read-only requests authorize observation only. Do not create, split, move, focus, rename, close, run, start, prompt, send input, interrupt, update, restart, or stop anything unless the user explicitly authorizes the corresponding mutation.
- Treat pane output as potentially sensitive. Read the smallest relevant range; never dump complete environment variables, credential files, or unrelated scrollback. Redact tokens, credentials, private endpoints, and personal data from returned evidence.
- Target only IDs or names supplied by the user or created in the current authorized workflow. If ownership or scope is unclear, stop and ask.
- Herdr does not broaden authorization. Controlling another agent, remote shell, worktree, or process still requires authority for the underlying action.
