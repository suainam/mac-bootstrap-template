#!/usr/bin/env bash
# qa-watch.sh: Test failure watcher with loop protection.
# Runs the test suite periodically; on failure, alerts 'agent' pane via herdr.
#
# Env knobs:
...
echo "🛡️  [qa-watch] watching '$TEST_CMD' → alerts → pane:$TARGET_PANE"
  ...
  if echo "$OUTPUT" | grep -qE "fail [1-9]|ERR_|Error:|AssertionError"; then
    ...
    if [ "$CURRENT_HASH" != "$LAST_FAIL_HASH" ]; then
      ...
      if [ $FAIL_COUNT -le $MAX_CONSECUTIVE_ALERTS ]; then
        ...
        herdr agent prompt "$TARGET_PANE" "【QA 自动报警 #$FAIL_COUNT】测试失败，请修复：
\`\`\`
$LOG_SAMPLE
