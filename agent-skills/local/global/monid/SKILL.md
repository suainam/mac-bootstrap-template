...
**Do not install or update automatically. Ask for explicit user approval before
any package-manager or skill-source change. Use the setup steps below only
after approval if ANY of these is true:**
...
2. Update this vendored skill only through the repository skill registry's
   approved source and hash flow. Never fetch and replace it directly from a
   URL during an agent turn.

Keep the CLI and skill on the same reviewed, pinned version. Do not use a
floating `latest` update to resolve a mismatch.
