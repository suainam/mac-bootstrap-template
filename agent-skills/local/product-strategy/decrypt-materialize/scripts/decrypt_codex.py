...
def check_codex_running() -> tuple[list[int], list[str]]:
    """检查 Codex 进程是否在运行，返回 (PIDs, 进程名列表)"""
    try:
        result = subprocess.run(
            ["pgrep", "-il", "codex"],  # -i 忽略大小写, -l 显示进程名
            capture_output=True,
            text=True,
            check=False
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="解密 Codex TSD 加密的数据库文件"
    )
    parser.add_argument(
        "codex_dir",
        type=Path,
        help="Codex 配置目录路径（通常是 ~/.codex）"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--stop-daemon",
        action="store_true",
        help="自动停止 Codex 守护进程（launchd）"
    )

    args = parser.parse_args()

    # 验证目录
    if not args.codex_dir.exists():
        print(f"错误: 目录不存在: {args.codex_dir}", file=sys.stderr)
        return 1
    ...
    if pids and not args.force:
        ...
        if args.stop_daemon:
            ...
            if stop_codex_daemon():
                print("已停止 launchd 服务，等待进程退出...", file=sys.stderr)
                import time
                time.sleep(2)
                # 强制杀掉残留进程
                for pid in pids:
    ...
    # 设置目录
    decrypted_dir = args.codex_dir / "decrypted"
    decrypted_dir.mkdir(exist_ok=True)

    backup_dir = args.backup_dir or (args.codex_dir / "backups")
