...
2. **RENAME 决策树**：
   - **Case A（旧表有生产分区，新表不存在）**：
     展示分区快照与回滚方案，获得用户明确确认后，执行 `ALTER TABLE <old_table> RENAME TO <new_table>;`，使历史分区无缝转移至正名表名下。
   - **Case B（新表已由他人新建为空表，旧表有分区）**：
     不得自动删除空新表；先保留并向用户展示两表状态。只有用户单独明确确认清理范围后，才可处理空新表，再将旧表 `RENAME` 到新表，防止历史资产孤立。
   - **Case C（两表均无或仅新表存在）**：
     直接在正名表下建表或写入。
3. **验证要求**：
