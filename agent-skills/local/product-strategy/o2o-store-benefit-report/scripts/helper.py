...
class O2OStoreBenefitRunner:
    def __init__(
        ...
        self.tag_source = tag_source.lower()  # 'new' for is_3he1_fl, 'raw' for reason_zfl
        self.target_project = target_project
        self.export_dir = export_dir or (REPO_ROOT / "topics/o2o_store/04_outputs/tables")
        ...
        self.connector = ODPSConnector(ODPSConfig(project=os.environ.get("ODPS_PROJECT", "dsl_ads")))
    ...
    def run_date_dimension(self) -> dict[str, Any]:
        ...
        if not self.version_date:
            ...
            with self.odps.execute_sql(sql_ver).open_reader() as reader:
                ...
                if not rec or rec[0] is None:
                    raise ValueError(f"Cannot auto-detect version online date prior to {self.cutoff_date}")
        ...
        , cfg as (
            select 
                ...
                add_months(c_s_date, -12) as h_s_date,
                add_months(c_e_date, -12) as h_e_date
            from base
        )
    ...
    def run_channel_sales(self, bounds: dict[str, Any]) -> None:
        ...
        for period in ["C", "L", "H"]:
            ...
            ;
            """
            self.execute_sql(sql_off_sub, f"Offline Sub-table Ingestion (Period {period})")
    ...
    def run_dws_and_ads(self, bounds: dict[str, Any]) -> None:
        ...
        with sales_agg as (
            select 
                case when is_key_store = 1 then '中心店' else 'O2O其他重点店' end as store_group,
            ...
            group by 
                case when is_key_store = 1 then '中心店' else 'O2O其他重点店' end,
                lx_new,
                period_type
            grouping sets (
                (case when is_key_store = 1 then '中心店' else 'O2O其他重点店' end, lx_new, period_type),
                (case when is_key_store = 1 then '中心店' else 'O2O其他重点店' end, lx_new, platform_name, period_type)
            )
        )
        , tag_turnover as (
            select 
                case when re.is_key_store = 1 then '中心店' else 'O2O其他重点店' end as store_group,
                ...
                count(a.item_code) as tag_item_cnt,
                count(case when a.lt90_item_sale_cnt_sum > 0 then a.item_code end) as tag_dx_item_cnt
            ...
            group by 
                case when re.is_key_store = 1 then '中心店' else 'O2O其他重点店' end,
                t2.is_3he1_fl,
                case when a.stat_date = {self.cutoff_date} then 'C'
                     when a.stat_date = {ver_id} then 'L'
                     when a.stat_date = {yoy_date} then 'H' end
        )
        , cata_turnover as (
            select 
                case when re.is_key_store = 1 then '中心店' else 'O2O其他重点店' end as store_group,
                ...
                sum(t0.item_num_ct) as cata_item_cnt,
                sum(t0.item_num_yxs_ct) as cata_dx_item_cnt
            ...
            group by 
                ...
                case when t0.stat_date = {self.cutoff_date} then 'C'
                     when t0.stat_date = {ver_id} then 'L'
                     when t0.stat_date = {yoy_date} then 'H' end
        )
        select 
            s.store_group,
            s.strategy_tag,
        ...
        left join tag_turnover t 
            on s.store_group = t.store_group and s.strategy_tag = t.strategy_tag and s.period_type = t.period_type
    ...
    def export_excel(self) -> str:
        ...
        where pt = {self.cutoff_date}
        order by 
            case when store_group = 'O2O其他重点店' then 1 else 2 end,
        ...
        if not rows:
            raise ValueError(f"No data returned from ads card table for pt={self.cutoff_date}")

        out_path = self.export_dir / f"o2o_key_store_benefit_cards_{self.cutoff_date}.xlsx"
        ...
        font_card_head = Font(name="Microsoft YaHei", size=11, bold=True, color="FFFFFF")
        ...
        fill_card_head = PatternFill(start_color="2F5597", end_color="2F5597", fill_type="solid")
        fill_tbl_head = PatternFill(start_color="41719C", end_color="41719C", fill_type="solid")
        fill_evidence_head = PatternFill(start_color="5B7065", end_color="5B7065", fill_type="solid")
        ...
        # Title block
        baseline_desc = "去年同期 (H期 同比)" if self.baseline_type == "H" else "上线前 (L期 环比)"
        ws.merge_cells("A1:P1")
        title_cell = ws["A1"]
        title_cell.value = f"O2O重点门店/中心店组货策略效益评估报告 (基期: {baseline_desc} | 评估截止: {self.cutoff_date})"
        ...
        for r in rows:
            key = (r["store_group"], r["strategy_tag"])
            cards.setdefault(key, []).append(r)

        curr_row = 3
        card_idx = 1
        for (s_group, s_tag), items in cards.items():
            # Card Banner
            ws.merge_cells(start_row=curr_row, start_column=1, end_row=curr_row, end_column=10)
            c_cell = ws.cell(row=curr_row, column=1, value=f"【卡片 {card_idx}】{s_group}「{s_tag}」效益评估")
            c_cell.font = font_card_head
            c_cell.fill = fill_card_head
            c_cell.alignment = align_center

            # Evidence Banner (Columns 11 to 16, no store count columns)
            ws.merge_cells(start_row=curr_row, start_column=11, end_row=curr_row, end_column=16)
            e_cell = ws.cell(row=curr_row, column=11, value="【店均规模量化证据栏】(防误判)")
            e_cell.font = font_card_head
            e_cell.fill = PatternFill(start_color="34495E", end_color="34495E", fill_type="solid")
            ...
            ws.merge_cells(start_row=curr_row, start_column=8, end_row=curr_row, end_column=10)
            ...
            ws.cell(row=curr_row, column=11, value="标签店均品数").alignment = align_center

            ws.merge_cells(start_row=curr_row, start_column=13, end_row=curr_row, end_column=14)
            ws.cell(row=curr_row, column=13, value="标签店均动销品数").alignment = align_center

            ws.merge_cells(start_row=curr_row, start_column=15, end_row=curr_row, end_column=16)
            ws.cell(row=curr_row, column=15, value="全店店均目录品数").alignment = align_center
            ...
            for item in items:
                ...
                m_cell = ws.cell(row=curr_row, column=1, value=f"{m_name}({unit})" if not is_dx else m_name)
                ...
                v_post = ws.cell(row=curr_row, column=2, value=f"{c_post:.2f}%" if is_dx and c_post is not None else (f"{c_post:.1f}万" if c_post is not None else "-"))
                v_pre = ws.cell(row=curr_row, column=3, value=f"{c_pre:.2f}%" if is_dx and c_pre is not None else (f"{c_pre:.1f}万" if c_pre is not None else "-"))
                v_diff = ws.cell(row=curr_row, column=4, value=f"{c_diff:+.2f}%" if is_dx and c_diff is not None else (f"{c_diff:+.1f}万" if c_diff is not None else "-"))
                ...
                if is_dx:
                    ws.merge_cells(start_row=curr_row, start_column=5, end_row=curr_row, end_column=10)
                    n_cell = ws.cell(row=curr_row, column=5, value=item.get("remark") or "")
                    ...
                    ws.cell(row=curr_row, column=5, value=f"{o_post:.1f}万" if o_post is not None else "-").alignment = align_right
                    ws.cell(row=curr_row, column=6, value=f"{o_pre:.1f}万" if o_pre is not None else "-").alignment = align_right
                    ws.cell(row=curr_row, column=7, value=f"{o_diff:+.1f}万" if o_diff is not None else "-").alignment = align_right

                    f_post = item.get("offline_post")
                    f_pre = item.get("offline_pre")
                    f_diff = item.get("offline_diff")
                    ws.cell(row=curr_row, column=8, value=f"{f_post:.1f}万" if f_post is not None else "-").alignment = align_right
                    ws.cell(row=curr_row, column=9, value=f"{f_pre:.1f}万" if f_pre is not None else "-").alignment = align_right
                    ws.cell(row=curr_row, column=10, value=f"{f_diff:+.1f}万" if f_diff is not None else "-").alignment = align_right
                ...
                ws.cell(row=curr_row, column=11, value=f"{ti_post:.1f}" if ti_post is not None else "-").alignment = align_right
                ws.cell(row=curr_row, column=12, value=f"{ti_pre:.1f}" if ti_pre is not None else "-").alignment = align_right

                td_post = item.get("c_avg_tag_dx")
                td_pre = item.get("l_avg_tag_dx")
                ws.cell(row=curr_row, column=13, value=f"{td_post:.1f}" if td_post is not None else "-").alignment = align_right
                ws.cell(row=curr_row, column=14, value=f"{td_pre:.1f}" if td_pre is not None else "-").alignment = align_right

                ci_post = item.get("c_avg_cata_items")
                ci_pre = item.get("l_avg_cata_items")
                ws.cell(row=curr_row, column=15, value=f"{ci_post:.1f}" if ci_post is not None else "-").alignment = align_right
                ws.cell(row=curr_row, column=16, value=f"{ci_pre:.1f}" if ci_pre is not None else "-").alignment = align_right
...
def main() -> None:
    parser = argparse.ArgumentParser(description="O2O Store Benefit Report Automation Runner")
    parser.add_argument("--cutoff", required=True, help="Evaluation cutoff date yyyyMMdd (e.g., yesterday 20260907)")
    parser.add_argument("--version", default=None, help="Assortment version online date yyyyMMdd (default: auto-detected)")
    parser.add_argument("--baseline", default="H", choices=["H", "L"], help="Baseline period type: H (YoY, default) or L (MoM)")
    parser.add_argument("--tag-source", default="new", choices=["new", "raw"], help="Tag priority source: new (is_3he1_fl, default) or raw (reason_zfl)")
    parser.add_argument("--export", action="store_true", help="Directly export formatted Excel after pipeline execution")
    parser.add_argument("--export-only", action="store_true", help="Only export formatted Excel from existing ADS partition without re-running ETL")
